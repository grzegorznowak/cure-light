...
- **Research enforcement (pi)**: when `research.mode: chhound-rail`, render the frame's exact prefixed tool names (`chh_pr<n>_daemon_status` / `_code_research` / `_search`) into every Vector-2/3 child's `{research_protocol}` slot (Variant A / Variant C in child-pass-prompt-template.md) — never generic aliases. Check each child's RESEARCH TRACE footer at the vector gate: missing/noncompliant trace = `inconclusive`, rerun once with the corrected rendered prompt, then operator gate. Traces are self-report — no child telemetry exists — so audit claimed `verified-correlated-sites` against the subject tree at `subject_oid`.
