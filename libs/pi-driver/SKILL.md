...
- The **chhound** rail (pi-chhound plugin) may or may not be available. When it is live (the coordinator's `ch-chhound status` probe; on older builds, install detected at boot + the operator's `/ch-status` report at the frame gate — chhound-driver.md), Phase 0 pulls the subject as a chunkhound PR sandbox (`ch-chhound {action: "worktree.create", pr: "<PR-URL>", connect: true}`; the operator's `/ch-worktree` + `/ch-mcp` are the fallback lane) and the connection exposes the `chh_*` tools to the session — the research rail for coordinator and children. Otherwise, git + bash + the repo itself are the always-available evidence base. Never block on a missing/broken rail — fall back to `git diff`/`rg`.
