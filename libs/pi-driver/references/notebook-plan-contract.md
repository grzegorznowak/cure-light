...
|---|---|---|---|
| `pipeline-frame-<owner>-<pr>-s<n>` | coordinator (seal → Phase 0) | frozen run options + planned subject mechanism (chhound sandbox | plain worktree) — no tree fields at the pre-pull gate; `subject_path` / `subject_oid`, changed-file list, contract ref (page or path), research binding (mode chhound-rail | direct-tree, `ch_prefix`, exact tool names, excluded namespaces, fallback reason), contract-source pins/designation, pinned toolchain (producer 0.3.0 / gate 0.2.0 / census 0.1.0 versions + sha256) and expected labeling mode (full | sliced), the source-consistency outcome (provisional `repair_required` / evidence-only authorization) and the post-V1 `review_basis` record, and fallback notes recorded at their gates once the pull lands | one review state; a deliberate re-pull or a contract-only repair starts a NEW frame (linked by diff), never overwritten in place |
...
| `claims-<owner>-<pr>-s<n>` | coordinator (Phase 0) | the **projection of the canonical claim registry** produced by the pinned `claim-registry` 0.3.0 run (published only under the state's `gate-check` permission): identical claim IDs/counts/`registry_hash`, per-source class/designating pointer/selection rule/interpretation (normative vs advisory, precedence), parent/group links, context/nonclaim labels, labeling-mode provenance (whole-source labels or reconciled bounded slices — window/slice IDs are never claim IDs) — plus the **source-consistency records** (conflicting quotes/offsets/source hashes/affected claim IDs, materiality witness) — the queryable complete claim directory for V1 negative attribution (paged `-p<k>` when it exceeds a page) | one review state, like the contract; canonical bytes kept per the artifact-store rule |
...
### Canonical artifact store (D2)

The run's canonical bytes — capture manifest + raw sources, frame/frame-slices manifests + payloads, every child proposal, merged proposals, reconciliation report, registry, validation report, run manifest, gate report, census artifact — are **content-addressed and verifiable**: each is stored with its sha256 (`sha256:<hex>` object content IDs, lowercase hex file digests) and each notebook projection is reassembled and hash-equality checked against those bytes before use. Two stores are allowed, and either (or a documented mix) may be used:
...
Whichever store holds the canonical bytes — notebook artifact pages or a durable run store — it is content-addressed and verifiable: run-store files used only as execution cache are not authoritative; notebook pages may hold the canonical bytes directly (paginated, content-addressed) under the same reassembly/hash-equality rule, and when the durable store is canonical the notebook pages are projections of it. **No pretty-JSON substitute:** a re-rendered summary, a pretty-printed copy or a partial excerpt never replaces the canonical bytes, and a projection whose reassembled bytes/hash differ from the canonical artifact is invalid. **Lifecycle:** canonical artifacts and their projections are kept through the state's closure/finalization window and retired only after a durable snapshot with matching hash lands (claims/coverage pages per their table rows); a retirement never leaves a live state page referencing unreachable bytes. A new review state gets a new `-s<n>` set and never overwrites or reuses the previous state's artifacts.
...
- **The claim registry is the complete claim universe — as a verified projection.** `claims-<owner>-<pr>-s<n>` renders the canonical producer registry compiled at Phase 0 (intake-and-scope.md §0.2): identical claim IDs, counts and `registry_hash`, reassembly/hash-equality checked against the canonical bytes, and behind the state's `gate-check` permission. A summary, a window/slice, a hand-renumbered set or a page without its backing canonical bytes is never a substitute; negative attribution that cannot query it returns `UNRESOLVED` (conformance-pass.md).
