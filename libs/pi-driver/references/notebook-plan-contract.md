...
| `claims-<owner>-<pr>-s<n>` | coordinator (Phase 0) | the compiled **claim registry**: deterministic claim IDs (source block + UTF-8 offsets + quote hash), parent/group links, context/nonclaim labels, source-window provenance — the queryable complete claim directory for V1 negative attribution (paged `-p<k>` when it exceeds a page) | one review state, like the contract |
