...
| 5 | Phase 0 | (chhound rail, when operator-confirmed) sandbox ready + bridge connected; prefixed tool responds — a first pull creates the sandbox (`/chworktree`) and connects it (`/ch-mcp`); a re-pull updates that same sandbox in place (bridge kept, reconnect only if it failed) | first pull: `/ch-mcp … --prefix chh_pr<n>`; re-pull: `git -C <subject_path> fetch/checkout`; then `chh_pr<n>_daemon_status` | reconnect once (operator); else record fallback (plain worktree + git/rg), never stop |
