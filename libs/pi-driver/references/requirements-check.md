...
| 4 | pre-pull | subject mechanism planned: pi-chhound installed (pi settings `packages` / extension dirs) + `chunkhound` CLI on PATH → planned subject = chunkhound PR sandbox **pending the operator's `/ch-status` confirmation at the frame gate** (unconfirmed → plain detached worktree); install not detected → plain detached worktree | inspect pi settings / extension dirs; `chunkhound` on PATH | record the fallback mechanism in the frame; never a stop by itself |
