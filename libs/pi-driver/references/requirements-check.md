...
| 5 | Phase 0 | (chhound rail, when operator-confirmed) sandbox/bridge ready; prefixed tool responds (an in-place re-pull retains the bridge) | first pull: `/ch-mcp … --prefix chh_pr<n>`; re-pull: `git -C <subject_path> fetch/checkout`; then `chh_pr<n>_daemon_status` | reconnect once (operator); else record fallback (plain worktree + git/rg), never stop |
