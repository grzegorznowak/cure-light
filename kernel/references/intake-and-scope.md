...
contract_sources: [{class, pointer, path/section, role, blob_subject, blob_base, hash, version_ref}]   # captured claim sources: PR body/issue version pins + explicitly designated in-diff sources (§0.2)
...
review_basis: {value: ready | limited-only | blocked | unknown, repair_required, blockers, allowed_next_scope, operator_disposition_ref}   # recorded after Vector 1 at the gate (conformance-pass.md), with the state's source/registry/coverage pins
