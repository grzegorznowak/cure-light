...
2. **Probe per symbol, never batched** (one symbol per query keeps every hit attributable).
   ...
   skip the chunk sample — the census (step 4) always runs; without chunks,
   occurrences are settled by reading their lines in the tree (step 6). For each
   ...
   never bound what must be inspected (step 4).
...
4. **Coverage census (always runs)** — enumerate every occurrence of each symbol with
   ...
   binary / vendored), and the exact command. The match unit is the **matching line**
   (one entry per line even when a line holds several matches); `\b` boundaries match
...
5. **Symbol map (the artifact)** — a header: owner/repo PR, review state, `base_oid` /
   ...
   **discovery-only**. Then the bounded heat table:
   ...
   consumer at the site, or listed as uninspected → the in-scope finding route
   ...
   every occurrence is tree-read and accounted, or routed as an uninspected finding.
