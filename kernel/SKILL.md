...
Pull the tree under review BEFORE any analysis or orientation in the target repo's checkouts: a chunkhound PR sandbox when the rail is live (the `ch-chhound status` model probe; on older builds, installed + the operator's `/ch-status` report at the frame gate), else a plain detached worktree (intake-and-scope.md §0.1). Until the pull, pre-pull work is remote-only (`gh repo view` / `gh pr view` / `gh pr diff --name-only`) plus presence probes (the `ch-chhound status` model probe; on older builds, pi-chhound install checks + the operator's `/ch-status` report at the frame gate); the only pre-pull local git command is the cure-light source provenance capture (intake-and-scope.md, §Output). Whatever SHA the pull has is the **subject** — capture it (`subject_oid` / `subject_path`) into the run manifest at Phase 0. Every fleet child of this review state receives the same subject path + OID. The tree is stable for the whole state — nothing mutates it mid-state; only a deliberate re-pull at an operator gate starts a new review state, which obeys the same subject-first rule and updates the tree **in place** at that boundary (intake-and-scope.md §0.1). A tree that is gone or broken is pulled fresh instead.
