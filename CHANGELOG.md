...
### v0.5.9 — the `blast` lens: data × call-site blast radius (V2-owned; advisory rows, blocking instances)

**kernel/**
- `blast-lens.md` (new) — a Vector 2-owned always-active lens distilled from an operator investigation of a data-dependent production bug: five rows — `data` (what pre-existing state reaches this change: legacy/orphan rows, dangling refs, NULL/empty, zero-value sentinels), `sweep` (a changed sentinel/constant/default obliges a call-site sweep **in the same PR**; a sweep deferred to a "follow-up ticket" is the hit), `semantics` (comparisons judged in the storage engine's value model, not the language's — column type + engine meaning checked at the site), `fixture` (a single-artificial-row trigger must be proven by a fixture test on the repo's own scratch DB, not by reading), `gates` (a "statically detectable" claim counts only where the analyzer/lint gate is actually enabled in the repo's own config/CI). Routing: rows are advisory (lens trail, LOW default, MED ceiling, never HIGH, notebook-only); the **concrete instance is a Vector 2 finding** at its own severity (HIGH possible) — the lens finds the class, the finding is what blocks; a PR deferring its own sweep is **in scope** (v0.5.8 route) → Findings, to be addressed. Determinism: `sweep` is mechanical (the repo's own literal/symbol sweep — always available, never installs), `semantics`/`gates` are schema/CI-config reads, `data`/`fixture` are judgment; the lens matrix renders it `partial`.
- `hygiene-lens.md` / `pipeline-model.md` / `intake-and-scope.md` / `implementation-pass.md` / `closure-verification.md` — the lens dimension points at the non-hygiene references; the matrix gains the `blast` row (`preflight + v2 split`, `partial`), the manifest example gains `blast: preflight+v2`, and the preflight schedule names the sentinel sweep. Vector 2's owned-lens section covers the five rows and the advisory-rows / blocking-instance split; closure revalidation covers `blast` rows (a concrete instance follows ordinary finding closure).
- `evidence-format.md` — schema line + severity bullet for `blast` rows (advisory, never HIGH) and the routing rule: lens rows stay in the notebook, the concrete instance is a bug-table finding.
...
- `child-pass-prompt-template.md` — the `YOUR LENSES` paragraph names the `blast` reference and splits advisory lens hits from a concrete `blast` instance (a Vector 2 finding); the `lenses` slot row lists the reference.

**README / BOOTSTRAP** — the lens paragraph, layout line, and statuses rule name the `blast` lens; the remote-boot fetch manifest gains `blast-lens.md` (entries renumbered).

Background: operator investigation of a data-dependent production bug — the code change and the legacy rows each looked harmless alone; only their interaction was a P1. The review must not skip: data in the blast radius, changed sentinels as API-wide changes (a deferred sweep is an admission of uninspected blast radius), storage-engine semantics at comparison sites, adversarial-fixture provability (reading finds the hazard class, only a test proves the instance), and wired static gates. Condensed into one five-row lens; the source's "must block" is mapped onto existing machinery instead of engine-level blocking: the lens stays advisory (cross-cutting rule 1), the concrete instance is a V2 finding at real severity, and the deferred-sweep case is already covered by the v0.5.8 scope route.
